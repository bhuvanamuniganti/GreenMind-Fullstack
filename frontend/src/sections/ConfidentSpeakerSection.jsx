// frontend/src/sections/ConfidentSpeakerSection.jsx
import React, { useEffect, useRef, useState } from "react";
import { API_BASE } from "../api";
import "./ConfidentSpeakerSection.css";
...
/* ----------------- MiniRecorder w/ waveform (unchanged) ----------------- */
...
export default function ConfidentSpeakerSection() {
  ...
  const [parentLang, setParentLang] = useState("hi");
  ...
  async function transcribeBlob(blob, name, itemId = null) {
    ...
    try {
      const r = await fetch(`${API_BASE}/api/practice-image/transcribe`, { method: "POST", body: form });
      if (!r.ok) {
    ...
    return (
      <section className="cs-card">
        ...
        <div className="cs-box">
          ...
          <p className="cs-hint">Waveform appears while recording — parent sees activity (visual cue).</p>
        </div>
        ...
        <div className="cs-box">
          ...
          <div className="cs-row" style={{ justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <select value={parentLang} onChange={(e)=>setParentLang(e.target.value)} className="cs-select-small">
  ...
  function FeedbackView() {
    ...
    return (
      <section className="cs-card">
        ...
        <div className="cs-box">
          <div className="label">Pronunciation hints (child → expected)</div>
          {diffs.length === 0 ? (
            <div className="cs-hint">Good match — no obvious word mismatches found.</div>
          ) : (
